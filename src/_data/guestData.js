...
// Point a /Guests/<file> path at its right-sized thumbnail in
// src/assets/guest-thumbs/ when one exists (generated by
// scripts/generate-guest-thumbs.mjs during prebuild/prestart). Falls back to
// the full-res original when the thumb is missing — e.g. a dev
// `eleventy --serve` before the generator has run. Doing the swap here, on the
// single `images` map, covers every consumer: both the server templates and
// the client-side data.js read this same map.
const THUMB_DIR = path.join(__dirname, '..', 'assets', 'guest-thumbs');
function preferThumb(imgPath) {
  if (!imgPath || !imgPath.startsWith('/Guests/')) return imgPath;
  const file = imgPath.slice('/Guests/'.length);
  return fs.existsSync(path.join(THUMB_DIR, file))
    ? '/assets/guest-thumbs/' + file
    : imgPath;
}
...
module.exports = function () {
  ...
  // Final pass: swap every resolved /Guests/ path for its thumbnail. Runs after
  // all sources (guestImages.json, profile JSON, episode frontmatter) are merged.
  for (const key of Object.keys(images)) {
    images[key] = preferThumb(images[key]);
  }
